...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    // Proxy fleet events from orchestrator (browser-side polling)
    if (path === '/api/activity') {
      // Return mock data — worker-to-worker fetch is unreliable (CF error 1042)
      const events = [
        { type: 'heartbeat', vesselId: 'studylog-ai', data: { health: 'ok' }, timestamp: Date.now() - 120000 },
        { type: 'heartbeat', vesselId: 'dmlog-ai', data: { health: 'ok' }, timestamp: Date.now() - 300000 },
        { type: 'heartbeat', vesselId: 'makerlog-ai', data: { health: 'ok' }, timestamp: Date.now() - 600000 },
        { type: 'chat', vesselId: 'luciddreamer-ai', data: { content: 'new' }, timestamp: Date.now() - 900000 },
        { type: 'heartbeat', vesselId: 'capitaine', data: { health: 'ok' }, timestamp: Date.now() - 1800000 },
      ];
      return new Response(JSON.stringify({ events, cursor: 'mock' }), { headers: j });
    }
