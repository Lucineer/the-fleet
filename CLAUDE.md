...
- **Vessel**: The Fleet
- **Role**: The Fleet
- **URL**: https://the-fleet.casey-digennaro.workers.dev
- **Repo**: github.com/Lucineer/the-fleet
- **Branch**: master
...
### Deploy
```bash
cd /tmp/the-fleet && wrangler deploy
```

### Health Check
```bash
curl -s https://the-fleet.casey-digennaro.workers.dev/health
curl -s https://the-fleet.casey-digennaro.workers.dev/vessel.json | python3 -m json.tool
```
...
- **Vessel Tuner**: https://vessel-tuner.casey-digennaro.workers.dev/api/vessel?name=the-fleet
...
- **vessel.json capabilities**: playground, vessels
...
1. Keep the vessel small. ~448 lines is the current size.
